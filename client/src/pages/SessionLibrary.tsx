    ...
    <div className="min-h-screen bg-background text-foreground p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-foreground/60">{sessions?.length ?? 0} sessions · All operational intelligence</p>
        </div>
            ...
            return (
              <div key={s.id} className="bg-muted/30 border border-border rounded-xl overflow-hidden">
                ...
                >
                  ...
                  <div className="flex items-center gap-2 text-foreground/60 text-xs w-28 flex-shrink-0">
                    <Calendar className="w-3 h-3" />{new Date(s.date).toLocaleDateString()}
                  </div>
                  <p className="text-sm text-foreground/80 flex-1 truncate">{s.executiveSummary}</p>
                  ...
                  <div className="px-5 pb-5 border-t border-border pt-4 grid grid-cols-1 lg:grid-cols-2 gap-4">
                    {participants.length > 0 && (
                      <div className="flex items-center gap-2 col-span-full">
                        ...
                        <span className="text-xs text-foreground/60">{participants.join(", ")}</span>
                      </div>
                    )}
                    {keyPoints.length > 0 && (
                      <div>
                        <p className="text-xs font-semibold text-foreground/70 uppercase tracking-wider mb-2">Key Points</p>
                    ...
                    {decisions.length > 0 && (
                      <div>
                        <p className="text-xs font-semibold text-foreground/70 uppercase tracking-wider mb-2">Decisions</p>
                    ...
                    {blockers.length > 0 && (
                      <div>
                        <p className="text-xs font-semibold text-foreground/70 uppercase tracking-wider mb-2">Blockers</p>
                    ...
                    {actions.length > 0 && (
                      <div>
                        <p className="text-xs font-semibold text-foreground/70 uppercase tracking-wider mb-2">Action Items</p>
