    ...
    <div className="min-h-screen bg-background text-foreground p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-foreground/40">{sessions?.length ?? 0} sessions · All operational intelligence</p>
        </div>
        <Button onClick={() => navigate("/voice")} className="bg-indigo-600 hover:bg-indigo-700 text-foreground gap-2 text-sm">
      ...
      <div className="relative mb-4">
        ...
        <Input
          ...
          className="pl-9 bg-muted/30 border-border text-foreground placeholder:text-foreground/30 focus:border-indigo-500"
        />
      </div>
            ...
            return (
              <div key={s.id} className="bg-muted/30 border border-border rounded-xl overflow-hidden">
                ...
                >
                  ...
                  <div className="flex items-center gap-2 text-foreground/40 text-xs w-28 flex-shrink-0">
                    <Calendar className="w-3 h-3" />{new Date(s.date).toLocaleDateString()}
                  </div>
                  <p className="text-sm text-foreground/70 flex-1 truncate">{s.executiveSummary}</p>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <Badge variant="outline" className="text-xs border-0 bg-muted/30 text-foreground/30">{s.meetingType}</Badge>
                    ...
                    {isOpen ? <ChevronUp className="w-4 h-4 text-foreground/30" /> : <ChevronDown className="w-4 h-4 text-foreground/30" />}
                  </div>
                </button>
                {isOpen && (
                  <div className="px-5 pb-5 border-t border-border pt-4 grid grid-cols-1 lg:grid-cols-2 gap-4">
                    {participants.length > 0 && (
                      <div className="flex items-center gap-2 col-span-full">
                        ...
                        <span className="text-xs text-foreground/40">{participants.join(", ")}</span>
                      </div>
                    )}
                    {keyPoints.length > 0 && (
                      <div>
                        <p className="text-xs font-semibold text-foreground/50 uppercase tracking-wider mb-2">Key Points</p>
                        <ul className="space-y-1">{keyPoints.map((kp, i) => <li key={i} className="text-xs text-foreground/60 flex gap-2"><span className="text-indigo-400">·</span>{kp}</li>)}</ul>
                      </div>
                    )}
                    {decisions.length > 0 && (
                      <div>
                        <p className="text-xs font-semibold text-foreground/50 uppercase tracking-wider mb-2">Decisions</p>
                        <ul className="space-y-1">{decisions.map((d, i) => <li key={i} className="text-xs text-foreground/60 flex gap-2"><span className="text-emerald-400">✓</span>{d}</li>)}</ul>
                      </div>
                    )}
                    {blockers.length > 0 && (
                      <div>
                        <p className="text-xs font-semibold text-foreground/50 uppercase tracking-wider mb-2">Blockers</p>
                        <ul className="space-y-1">{blockers.map((b, i) => <li key={i} className="text-xs text-foreground/60 flex gap-2"><span className="text-red-400">!</span>{b}</li>)}</ul>
                      </div>
                    )}
                    {actions.length > 0 && (
                      <div>
                        <p className="text-xs font-semibold text-foreground/50 uppercase tracking-wider mb-2">Action Items</p>
                        <ul className="space-y-1">{(actions as Array<string | { task?: string }>).map((a, i) => <li key={i} className="text-xs text-foreground/60 flex gap-2"><span className="text-yellow-400">→</span>{typeof a === "string" ? a : (a.task ?? JSON.stringify(a))}</li>)}</ul>
                      </div>
                    )}
                  </div>
                )}
